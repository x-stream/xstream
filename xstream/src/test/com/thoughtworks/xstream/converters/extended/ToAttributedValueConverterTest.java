...
public class ToAttributedValueConverterTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        final ClassAliasingMapper classAliasingMapper = new ClassAliasingMapper(new DefaultMapper(
            new ClassLoaderReference(getClass().getClassLoader())));
    ...
    public void testWithValueInConvertedClass() {
        converterLookup.registerConverter(new ToAttributedValueConverter(Software.class, mapper, reflectionProvider,
            converterLookup, "name"), 0);
        ...
        try (final HierarchicalStreamReader reader = driver.createReader(new StringReader(writer.toString()))) {
    ...
    public void testWithValueInSuperclass() {
        converterLookup.registerConverter(new ToAttributedValueConverter(OpenSourceSoftware.class, mapper,
            reflectionProvider, converterLookup, "name", Software.class), 0);

        final Software software = new OpenSourceSoftware("Codehaus", "XStream", "BSD");
        final StringWriter writer = new StringWriter();
        try (final CompactWriter compactWriter = new CompactWriter(writer)) {
    ...
    public void testWillDistinguishBetweenDifferentTypes() {
        converterLookup.registerConverter(new ToAttributedValueConverter(Software.class, mapper, reflectionProvider,
            converterLookup, "name"), 0);
        converterLookup.registerConverter(new ToAttributedValueConverter(OpenSourceSoftware.class, mapper,
            reflectionProvider, converterLookup, "license"), 0);

        final Software[] software = new Software[]{
            new Software("Microsoft", "Windows"), new OpenSourceSoftware("Codehaus", "XStream", "BSD")};
        final StringWriter writer = new StringWriter();
        try (final PrettyPrintWriter prettyPrintWriter = new PrettyPrintWriter(writer)) {
        ...
        try (final HierarchicalStreamReader reader = driver.createReader(new StringReader(writer.toString()))) {
            final Software[] array = (Software[])new TreeUnmarshaller(null, reader, converterLookup, mapper).start(
                null);
    ...
    public void testWithComplexValueField() {
        converterLookup.registerConverter(new ToAttributedValueConverter(X.class, mapper, reflectionProvider,
            converterLookup, "innerObj"), 0);
